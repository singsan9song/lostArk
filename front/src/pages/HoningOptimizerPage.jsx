...
export default function HoningOptimizerPage() {
  ...
  const selectedItem =
    resultItems.find((item) => item.id === activeEquipment) || resultItems[0] || equipment[2]
  ...
  const estimatedLevel =
    1660 +
    (equipment.reduce((sum, item) => sum + calculated[item.id].target, 0) / equipment.length) *
      6.666
          ...
          <div className="honing-material-options">
            <label>
              <input
                type="checkbox"
                checked={useFlame}
                onChange={(event) => setUseFlame(event.target.checked)}
              />{' '}
              운명의 업화
            </label>
            <label>
              <input
                type="checkbox"
                checked={useThrill}
                onChange={(event) => setUseThrill(event.target.checked)}
              />{' '}
              운명의 전율
            </label>
          </div>
          ...
          <article className="honing-result-card panel">
            ...
            <div className="honing-average-cost">
              <div>
                <small>평균 비용</small>
                <strong>
                  <GoldAmount>{number(101930 * scale)}</GoldAmount>
                </strong>
                <span>
                  <i className="silver-dot" /> {number(238666 * scale)}
                </span>
              </div>
              ...
              {materialDefinitions.map(({ name, quantity, value, Icon }) => (
                <div key={name}>
                  ...
                  <span>
                    <small>{name}</small>
                    <b>
                      {number(quantity * scale)}
                      {name === '누적 골드' ? '' : '개'}
                    </b>
                  </span>
                  <strong>
                    <GoldAmount>{number(value * scale)}</GoldAmount>
                  </strong>
                </div>
              ))}
            </div>
            ...
            <div className="honing-segments" role="tablist" aria-label="재련 구간">
              {Array.from({ length: stepCount }, (_, index) => selectedSetting.current + index).map(
                (stage) => (
                  <button
                    type="button"
                    key={stage}
                    className={activeSegment === stage ? 'active' : ''}
                    onClick={() => setActiveSegment(stage)}
                  >
                    {stage}강 → {stage + 1}강
                  </button>
                ),
              ...
              <header>
                ...
                <div className="honing-route-summary">
                  ...
                  <span>
                    <small>평균 비용</small>
                    <b>
                      <GoldAmount>35,959</GoldAmount>
                    </b>
                    <em>
                      <i className="silver-dot" /> 87,622
                    </em>
                  </span>
                  <span>
                    <small>장기백 비용</small>
                    <b>
                      <GoldAmount>81,257</GoldAmount>
                    </b>
                    <em>
                      <i className="silver-dot" /> 198,000
                    </em>
                  </span>
                </div>
              </header>
