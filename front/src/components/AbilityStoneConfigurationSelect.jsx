...
export default function AbilityStoneConfigurationSelect({
  data,
  loading,
  selectedId,
  onSelect,
  ...
  return (
    <div className={`ability-stone-choice ${open ? 'expanded' : ''}`} ref={rootRef}>
      <button
        className="ability-stone-choice-current"
        type="button"
        aria-expanded={open}
        onClick={toggle}
      ...
      >
        ...
        <strong>
          {loading || !selected ? (
            ...
            <GoldAmount>{formatGold(selected.currentMinPrice)}</GoldAmount>
          )}
        </strong>
        <ChevronDown />
      </button>
      <div className="ability-stone-choice-motion" aria-hidden={!open}>
        <div>
          <section
            className="ability-stone-choice-options"
            aria-label="어빌리티 스톤 가격 조합 선택"
          >
              ...
              return (
                ...
                >
                  ...
                  <span>
                    ...
                    <small>
                      {(configuration.engravings || []).join(' + ')} · {configuration.penalty}
                    </small>
                  </span>
                  {active && (
                    <em>
                  ...
                  <strong>
                    <GoldAmount>{formatGold(configuration.currentMinPrice)}</GoldAmount>
                  </strong>
                </button>
              )
            })}
