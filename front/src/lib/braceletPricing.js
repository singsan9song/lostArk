...
export function buildBraceletValueBandMatrix(pairId) {
  ...
  return firstDistribution.flatMap((firstBand) =>
    secondDistribution.map((secondBand) => ({
...
export function calculateBraceletPairExpectedPrice(
  pairId,
  pricesByValueBand,
  policy = braceletData.auctionPricingPolicy,
) {
  const matrix = buildBraceletValueBandMatrix(pairId)
  const cells = matrix.map((cell) => {
    const prices = pricesByValueBand[cell.key] || []
    const averagePrice =
      prices.length >= policy.minimumSampleSize
  ...
  const availableProbabilityPercent = availableCells.reduce(
    (sum, cell) => sum + cell.probabilityPercent,
    0,
  )
  ...
  return {
    ...
    partialCoverageEstimate:
      availableProbabilityPercent > 0
...
export function calculateAllBraceletPairPrices(pricesByPairAndValueBand) {
  return braceletData.validFixedOptionPairs
    .map((pair) =>
      calculateBraceletPairExpectedPrice(pair.id, pricesByPairAndValueBand[pair.id] || {}),
    )
    .sort(
      (a, b) =>
        (b.expectedPrice ?? b.partialCoverageEstimate ?? -1) -
        (a.expectedPrice ?? a.partialCoverageEstimate ?? -1),
    )
}
