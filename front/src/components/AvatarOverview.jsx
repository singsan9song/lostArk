...
function normalizeType(type = '') {
  return type.replace(' 아바타', '').replace('무기 덧입기', '무기')
}

export default function AvatarOverview({ profile, avatars }) {
  const tendencies = Object.fromEntries(
    (profile.Tendencies || []).map((item) => [item.Type, item.Point]),
  )
  const slots = slotOrder.map((type) => ({
    type,
    item: avatars.find((avatar) => normalizeType(avatar.Type).includes(type)),
  }))
  ...
  const dyedItems = avatars
    .map((item) => ({ item, colors: colorsFromTooltip(item.Tooltip) }))
    .filter((entry) => entry.colors.length)

  return (
    <div className="avatar-dashboard">
      <section className="tendency-bar">
        {['지성', '담력', '친절', '매력'].map((name) => (
          <div key={name}>
            <span>{name}</span>
            <strong>{tendencies[name] ?? 0}</strong>
          </div>
        ))}
      </section>
      <section className="avatar-stage">
        <div className="avatar-stage-glow" />
        <div className="avatar-slots left">
          {leftSlots.map((slot) => (
            <AvatarSlot {...slot} key={slot.type} />
          ))}
        </div>
        ...
        <div className="avatar-slots right">
          {rightSlots.map((slot) => (
            <AvatarSlot {...slot} reverse key={slot.type} />
          ))}
        </div>
      </section>
      <section className="dye-section">
        <div className="avatar-section-heading">
          <div>
            <Palette />
            <h2>염색 정보</h2>
          </div>
          <span>{dyedItems.length ? `${dyedItems.length}개 부위` : '염색 정보 없음'}</span>
        </div>
        {dyedItems.length ? (
            ...
            {dyedItems.map(({ item, colors }, index) => (
              <article key={`${item.Type}-${index}`}>
                ...
                {colors.map((color, colorIndex) => (
                  <div className="dye-row" key={color}>
                    <span>부위{colorIndex + 1}</span>
